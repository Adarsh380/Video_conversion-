...
[![Live Demo](https://img.shields.io/badge/Live-Demo-blue)](http://localhost:3000) [![Pipeline](https://img.shields.io/badge/Pipeline-7%20Stages-green)](#pipeline) [![RAG](https://img.shields.io/badge/RAG-Enhanced-purple)](#rag-implementation) ![Version](https://img.shields.io/badge/Version-2.0-blue) ![License](https://img.shields.io/badge/License-MIT-green)

## 🚀 Features

### **Professional 7-Stage Pipeline**
- **Stage A**: Document Parsing - Extract text from PDF, DOCX, TXT, PPTX, XLSX, Images (OCR)
- **Stage B**: Scene Generation - AI-powered 5-20 scene creation based on content length
- **Stage C**: Visual Keywords - RAG-enhanced pattern matching and query optimization
- **Stage D**: Asset Fetching - Smart video asset discovery with library caching
- **Stage E**: Narration Creation - Text-to-speech generation
- **Stage F**: Video Assembly - Scene composition and transitions
- **Stage G**: Export - Multi-format delivery (MP4, WebM, MOV ProRes)

### **RAG Implementation** 
🧠 **Intelligent Visual Pattern Matching**
- Vector-based retrieval from visual pattern knowledge base
- Multi-factor re-ranking: similarity (40%) + semantics (30%) + mood (20%) + keywords (10%)
- Context-aware query generation for stock video APIs

### **Smart Asset Management**
- Embedding-based asset reuse (≥0.8 similarity threshold)
- Pexels & Pixabay VIDEO API integration
- Automatic library caching and metadata tracking
...
### **3. Convert Documents**
1. Navigate to converter via "Launch App"
2. Select document type and upload file
3. Choose output format
4. Watch 7-stage pipeline process your content
5. Download enhanced JSON with complete metadata

## 🏗️ Architecture

### **Frontend**
- **Homepage**: Professional landing page with feature showcase
- **Converter**: Clean UI with real-time progress tracking
- **Processing**: Extended loading feedback with stage visualization

### **Backend Pipeline**
```
Document Input → Parse (A) → Generate Scenes (B) → Refine Keywords (C) → Fetch Assets (D) → Create Audio (E) → Assemble (F) → Export (G) → Video Output
```

### **Data Management**
```
data/
├── visual_patterns.json     # 10 visual patterns for RAG retrieval
└── asset_library.json       # Smart asset caching system

src/pipeline/
├── parseDocument.ts         # Stage A - Multi-format document parsing
├── generateScenes.ts        # Stage B - AI scene segmentation
├── visualKeywords.ts        # Stage C - RAG-enhanced keyword refinement
└── fetchAssets.ts          # Stage D - Intelligent asset discovery
```
...
### **Knowledge Retrieval**
```typescript
// Vector-based pattern retrieval
const candidates = ragRetrievePatterns(queryEmbedding, topK * 2);

// Context-aware re-ranking  
const reranked = ragRerankPatterns(candidates, scene, topK);
```

### **Multi-Factor Scoring**
- **Vector Similarity**: Cosine similarity between scene and pattern embeddings
- **Semantic Relevance**: Word overlap and context matching using Jaccard similarity
- **Mood Alignment**: Scene mood compatibility scoring
- **Keyword Overlap**: Direct keyword matching for precision

### **Query Optimization**
- Platform-specific queries for Pexels and Pixabay APIs
- Backup query generation for fallback scenarios
...
A simple document-to-video web app with a static landing page (index.html) and a functional converter (video-converter.html).

Run locally (port 400):
- npm install
- npm start

Open:
- http://localhost:400/index.html
- http://localhost:400/video-converter.html

Notes:
- The static server is server.js (Node HTTP). No build required.
- Optional Next.js dev (not required for the static pages): npm run dev

Update GitHub repository:
1) git init
2) git remote add origin https://github.com/Adarsh380/Video_conversion-.git
3) git add -A
4) git commit -m "Setup static server and converter page"
5) git branch -M main
